    ...
    public class VirusTestRepository : IVirusTestRepository
    {
        ...
        { 
            ...
            if (!Directory.Exists(rootPath))
                ...
                var filePath = Path.Combine(rootPath, testFile.VirusTestFile.FileName);
                using (var stream = new FileStream(filePath, FileMode.Create))
                {
                    currentPatient.VirusTestFile = testFile.VirusTestFile.FileName;

                    await testFile.VirusTestFile.CopyToAsync(stream);
